...
try {
    switch ($test_type.ToLower()) {
        ...
        "vst3val" {
            $proc = Start-Process -PassThru -Wait -NoNewWindow "$binaries_dir\VST3-Validator.exe" "$binaries_dir\Mirage.vst3"
            if ($proc.ExitCode -ne 0) { throw "" }
        }
        "pluginval" {
            ...
            $proc = Start-Process -PassThru -Wait -NoNewWindow "pluginval.exe" "--verbose","--validate","$binaries_dir\Mirage.vst3"
            if ($proc.ExitCode -ne 0) { throw "" }
        }
        "clapval" {
            ...
            $proc = Start-Process -PassThru -Wait -NoNewWindow "clap-validator.exe"  "validate","$binaries_dir\Mirage.clap"
            if ($proc.ExitCode -ne 0) { throw "" }
        }
        default {
