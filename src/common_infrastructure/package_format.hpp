...
struct PackageJob {
    enum class State {
        ...
        Done, // worker thread is not running, packages install completed (or failed)
    ...
    struct Folder {
        ...
        ValueOrError<FolderCheckResult, PackageError> check_result;
        UserDecision user_decision {};
    };
    ArenaList<Folder, false> folders {arena};
};

PUBLIC void CompleteJob(PackageJob& job);

PUBLIC void StartJob(PackageJob& job) {
    ASSERT(job.state.Load(LoadMemoryOrder::Relaxed) == PackageJob::State::Installing);

    job.file_reader = ({
        auto o = Reader::FromFile(job.path);
        if (o.HasError()) {
            job.error_log.Error({}, "{}", o.Error());
            job.error = true;
            job.state.Store(PackageJob::State::Done, StoreMemoryOrder::Release);
            return;
        }
        o.ReleaseValue();
    });
    ...
    if (auto const init_o = ReaderInit(*job.reader, job.error_log); init_o.HasError()) {
        job.error = true;
        job.state.Store(PackageJob::State::Done, StoreMemoryOrder::Release);
        return;
    }
    ...
    bool all_ready_to_install = true;
    while (true) {
        auto const folder = ({
            auto const o = IteratePackageFolders(*job.reader, it, job.arena, job.error_log);
            if (o.HasError()) {
                job.error = true;
                job.state.Store(PackageJob::State::Done, StoreMemoryOrder::Release);
                return;
        ...
        Optional<ValueOrError<FolderCheckResult, PackageError>> check_result_opt {};
        switch (folder->type) {
            case package::SubfolderType::Libraries: {
                // TODO: we need the sample library server to see if this library is already installed
                check_result_opt =
                    LibraryCheckExistingInstallation(*folder, nullptr, job.arena, job.error_log);
                break;
            }
            case package::SubfolderType::Presets: {
                // TODO: we need the preset folders to check if these presets are already installed
                check_result_opt = PresetsCheckExistingInstallation(*folder, {}, job.arena, job.error_log);
                break;
            }
            case package::SubfolderType::Count: PanicIfReached();
        }

        auto check_result = check_result_opt.ReleaseValue();

        if (check_result.HasError())
            all_ready_to_install = false;
        else if (auto r = check_result.Get<FolderCheckResult>();
                 r.recommended_action == RecommendedAction::AskUser) {
            all_ready_to_install = false;
        }
        ...
        PackageJob::Folder {
            ...
            .user_decision = PackageJob::UserDecision::Unknown,
        };
    }

    if (!all_ready_to_install) {
        job.state.Store(PackageJob::State::AwaitingUserInput, StoreMemoryOrder::Release);
        return;
    }
...
PUBLIC void CompleteJob(PackageJob& job) {
    ...
    for (auto& folder : job.folders) {
        ASSERT(!folder.check_result.HasError());
        auto& check_result = folder.check_result.Get<FolderCheckResult>();
        if (check_result.recommended_action == RecommendedAction::AskUser)
            ASSERT(folder.user_decision != PackageJob::UserDecision::Unknown);

        if (folder.user_decision == PackageJob::UserDecision::Skip)
            continue;
        else if (folder.user_decision == PackageJob::UserDecision::Overwrite)
            check_result.extract_options.overwrite_existing_files = true;
        ...
        if (auto const o = ReaderExtractFolder(*job.reader,
                                               folder.folder,
                                               destination_folder,
                                               job.arena,
                                               job.error_log,
                                               check_result.extract_options);
            o.HasError()) {
            job.error = true;
            job.state.Store(PackageJob::State::Done, StoreMemoryOrder::Release);
            return;
