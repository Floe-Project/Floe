...
ErrorCodeOr<void> Main(String destination_folder) {
...

    {
        ...
        g_stdout_log.InfoLn({}, "Generating {}", lua_path);
...

    {
        ...
        g_stdout_log.InfoLn({}, "Generating {}", mdbook_config_path);
...
int main(int argc, char** argv) {
    ...
    if (argc != 2) {
        g_stdout_log.ErrorLn({}, "Usage: {} <destination_folder>", argv[0]);
        return 1;
    }
    ...
    if (result.HasError()) {
        g_stdout_log.ErrorLn({}, "Error: {}", result.Error());
        return 1;
    }
