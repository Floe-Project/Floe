...
consteval AtTimes MakeAtTimes() {
    AtTimes result;
    result.t[0] = 1;
    for (int i = 1; i < 121; ++i) {
        auto const fi = (f32)i;
        result.t[i] = ((0.08924f / fi) + (0.60755f / (fi * fi)) - 0.00006f);
    }
    return result;
}

constexpr AtTimes k_attimes = MakeAtTimes();
...
struct StillwellMajorTom {
    ...
    void Process(f32 srate, f32 spl0, f32 spl1, f32& out_spl0, f32& out_spl1) {
        ...
        if (overdb > maxover) {
            maxover = overdb;
            attime = k_attimes.t[(int)Max(0.0f, Floor(Fabs(overdb)))]; // attack time per formula
