...
PUBLIC Style MenuItem(imgui::Context const& imgui, bool _closes_popups, bool greyed_out = false) {
    ...
    s.back_cols.reg = 0;
    s.back_cols.hot_on = greyed_out ? 0 : LiveCol(imgui, UiColMap::PopupItemBackHover);
    s.back_cols.hot_off = greyed_out ? 0 : s.back_cols.hot_on;
    s.back_cols.active_on = greyed_out ? 0 : LiveCol(imgui, UiColMap::PopupItemBackHover);
    s.back_cols.active_off = s.back_cols.active_on;
