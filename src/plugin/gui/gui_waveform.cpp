...
static void GUIDoSampleWaveformOverlay(Gui* g, PluginInstance::Layer* layer, Rect r, Rect waveform_r) {
    ...
    auto const handle_height = editor::GetSize(imgui, UiSizeId::Main_WaveformHandleHeight);
    auto const handle_width = editor::GetSize(imgui, UiSizeId::Main_WaveformHandleWidth);
    ...
    if (loop_points_editable) {
        ...
        if (loop_points_editable) {
            ...
            grabber.w += extra_grabbing_room_x;
            if (reverse && !ping_pong) grabber.x -= extra_grabbing_room_x;

            if (xfade_active) {
                bool const invert = !ping_pong ? !reverse : false;
