...
static VoidOrError<Error> TryRunLuaCode(LuaState& ctx, int r) {
    ...
    switch (r) {
        ...
        case LUA_ERRERR:
            g_log.ErrorLn("🐛: error while running the error handler function");
    ...
    try {
        ...
        *(LuaState**)lua_getextraspace(ctx.lua) = &ctx;
        ...
        if (auto const r = luaL_loadbuffer(ctx.lua,
            ...
            g_log.ErrorLn("🐛: unknown error from lua_load: {}", r);
            return Error {LuaErrorCode::Unexpected, {}};
        }

        if (!lua_isfunction(ctx.lua, -1)) {
            g_log.ErrorLn("🐛: we're expecting the lua file to be a function");
            return Error {LuaErrorCode::Unexpected, {}};
        }
...
TEST_CASE(TestDocumentedExampleIsValid) {
    ...
    if (auto err = o.TryGet<Error>()) tester.log.ErrorLn("Error: {}, {}", err->code, err->message);
    CHECK(o.Is<Library*>());
...
TEST_CASE(TestIncorrectParameters) {
    ...
    auto check_error = [&](String lua) {
        ...
        if (o.Is<Error>()) {
            ...
            tester.log.ErrorLn("Error: not expecting this code to succeed: {}", lua);
    };
...
TEST_CASE(TestAutoMapKeyRange) {
    ...
    SUBCASE("2 files") {
        ...
        if (auto err = r.TryGet<Error>()) tester.log.ErrorLn("Error: {}, {}", err->code, err->message);
        REQUIRE(!r.Is<Error>());
    ...
    auto check = [&](ErrorCodeOr<void> expected, String lua_code, Options options = {}) {
        ...
        if (auto err = outcome.TryGetFromTag<ResultType::Error>()) {
            if (expected.Succeeded()) {
                ...
                tester.log.DebugLn("Success: failure expected: {d}", err->code);
            }
