...
void ComplexMultiplyAccumulate(Sample *FFTCONVOLVER_RESTRICT re,
    ...
    const size_t end4 = 4 * (len / 4);
    for (size_t i = 0; i < end4; i += 4) {
        const auto ra = vld1q_f32(&reA[i]);
        const auto rb = vld1q_f32(&reB[i]);
        const auto ia = vld1q_f32(&imA[i]);
        const auto ib = vld1q_f32(&imB[i]);
        auto real = vld1q_f32(&re[i]);
        auto imag = vld1q_f32(&im[i]);
        real = vaddq_f32(real, vmulq_f32(ra, rb));
        real = vsubq_f32(real, vmulq_f32(ia, ib));
        vst1q_f32(&re[i], real);
        imag = vaddq_f32(imag, vmulq_f32(ra, ib));
        imag = vaddq_f32(imag, vmulq_f32(ia, rb));
        vst1q_f32(&im[i], imag);
    }
    for (size_t i = end4; i < len; ++i) {
        re[i] += reA[i] * reB[i] - imA[i] * imB[i];
