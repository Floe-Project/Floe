...
DEPRECATED APIS
===============
Version 0.9.0 deprecated the per-sample reading and seeking APIs and replaced them with versions that work on
the resolution of a PCM frame instead. For example, given a stereo WAV file, previously you would pass 2 to
drwav_read_f32() to read one PCM frame, whereas now you would pass in 1 to drwav_read_pcm_frames_f32(). The
old APIs would return the number of samples read, whereas now it will return the number of PCM frames. Below
is a list of APIs that have been deprecated and their replacements.
...
Rationale:
    1) Most programs will want to read in multiples of the channel count which demands a per-frame reading
API. Per-sample reading just adds complexity and maintenance costs for no practical benefit. 2) This is
consistent with my other decoders - dr_flac and dr_mp3.

These APIs will be removed completely in version 0.10.0. You can continue to use drwav_read_raw() if you need
per-sample reading.
*/
...
//     #include "dr_wav.h"
//
// You can then #include this file in other parts of the program as you would with any other header file. Do
// something like the following to read audio data:
//
//     drwav wav;
...
//     }
//
//     drwav_int32* pDecodedInterleavedSamples = malloc(wav.totalPCMFrameCount * wav.channels *
//     sizeof(drwav_int32)); size_t numberOfSamplesActuallyDecoded = drwav_read_pcm_frames_s32(&wav,
//     wav.totalPCMFrameCount, pDecodedInterleavedSamples);
//
//     ...
//
//     drwav_uninit(&wav);
...
//     drwav_close(pWav);
//
// If you just want to quickly open and read the audio data in a single operation you can do something like
// this:
//
...
//     &totalPCMFrameCount); if (pSampleData == NULL) {
...
//
// The examples above use versions of the API that convert the audio data to a consistent format (32-bit
// signed PCM, in this case), but you can still output the audio data in its internal format (see notes below
// for supported formats):
//
//     size_t samplesRead = drwav_read_pcm_frames(&wav, wav.totalPCMFrameCount, pDecodedInterleavedSamples);
//
// You can also read the raw bytes of audio data, which could be useful if dr_wav does not have native support
// for a particular data format:
//
//     size_t bytesRead = drwav_read_raw(&wav, bytesToRead, pRawDataBuffer);
//
//
// dr_wav can also be used to output WAV files. This does not currently support compressed formats. To use
// this, look at drwav_open_write(), drwav_open_file_write(), etc. Use drwav_write_pcm_frames() to write
// samples, or drwav_write_raw() to write raw data in the "data" chunk.
//
...
//
//
// dr_wav has seamless support the Sony Wave64 format. The decoder will automatically detect it and it should
// Just Work without any manual intervention.
//
//
// OPTIONS
...
//   and drwav_read_pcm_frames_s16() to read and convert audio data to 32-bit floating point, signed 32-bit
//   integer and signed 16-bit integer samples respectively. Tested and supported internal formats include the
//   following:
//   - Unsigned 8-bit PCM
//   - Signed 12-bit PCM
//   - Signed 16-bit PCM
...
// - dr_wav will try to read the WAV file as best it can, even if it's not strictly conformant to the WAV
// format.
...
//
// Whether or not it is relative to the beginning or current position is determined by the "origin" parameter
// which will be either drwav_seek_origin_start or drwav_seek_origin_current.
typedef drwav_bool32 (*drwav_seek_proc)(void *pUserData, int offset, drwav_seek_origin origin);

// Callback for when drwav_init_ex/drwav_open_ex finds a chunk.
//
// pChunkUserData    [in] The user data that was passed to the pChunkUserData parameter of drwav_init_ex(),
// drwav_open_ex() and family. onRead            [in] A pointer to the function to call when reading. onSeek
// [in] A pointer to the function to call when seeking. pReadSeekUserData [in] The user data that was passed
// to the pReadSeekUserData parameter of drwav_init_ex(), drwav_open_ex() and family. pChunkHeader      [in] A
// pointer to an object containing basic header information about the chunk. Use this to identify the chunk.
//
// Returns the number of bytes read + seeked.
//
// To read data from the chunk, call onRead(), passing in pReadSeekUserData as the first parameter. Do the
// same for seeking with onSeek(). The return value must be the total number of bytes you have read _plus_
// seeked.
//
...
typedef struct {
    ...
    // The size of the extended data. Only used internally for validation, but left here for informational
    // purposes.
    drwav_uint16 extendedSize;

    // The number of valid bits per sample. When <formatTag> is equal to WAVE_FORMAT_EXTENSIBLE,
    // <bitsPerSample> is always rounded up to the nearest multiple of 8. This variable contains information
    // about exactly how many bits a valid per sample. Mainly used for informational purposes.
...
typedef struct {
    ...
    // A pointer to the function to call when data needs to be written. Only used when the drwav object is
    ...
    // initialization time. Always set to 0 for non-sequential writes and when the drwav object is opened in
    // read mode. Used for validation.
    ...
    struct {
        drwav_uint64 iCurrentSample; // The index of the next sample that will be read by drwav_read_*(). This
                                     // is used with "totalSampleCount" to ensure we don't read excess samples
...
// onRead                       [in]           The function to call when data needs to be read from the
// client. onSeek                       [in]           The function to call when the read position of the
// client data needs to move. onChunk                      [in, optional] The function to call when a chunk is
// enumerated at initialized time. pUserData, pReadSeekUserData [in, optional] A pointer to application
// defined data that will be passed to onRead and onSeek. pChunkUserData               [in, optional] A
// pointer to application defined data that will be passed to onChunk. flags                        [in,
// optional] A set of flags for controlling how things are loaded.
//
...
//
// If you want dr_wav to manage the memory allocation for you, consider using drwav_open() instead. This will
// allocate a drwav object on the heap and return a pointer to it.
//
// Possible values for flags:
//   DRWAV_SEQUENTIAL: Never perform a backwards seek while loading. This disables the chunk callback and will
//   cause this function
//                     to return as soon as the data chunk is found. Any chunks after the data chunk will be
//                     ignored.
//
// drwav_init() is equivalent to "drwav_init_ex(pWav, onRead, onSeek, NULL, pUserData, NULL, 0);".
//
// The onChunk callback is not called for the WAVE or FMT chunks. The contents of the FMT chunk can be read
// from pWav->fmt after the function returns.
//
...
// drwav_init_memory() to open the stream from a file or from a block of memory respectively.
//
// If the total sample count is known, you can use drwav_init_write_sequential(). This avoids the need for
// dr_wav to perform a post-processing step for storing the total sample count and the size of the data chunk
// which requires a backwards seek.
//
// If you want dr_wav to manage the memory allocation for you, consider using drwav_open() instead. This will
// allocate a drwav object on the heap and return a pointer to it.
//
...
//
// You can also use drwav_open_file() and drwav_open_memory() to open the stream from a file or from a block
// of memory respectively.
//
// This is different from drwav_init() in that it will allocate the drwav object for you via DRWAV_MALLOC()
...
// Close the loader with drwav_close().
//
// You can also use drwav_open_file_write() and drwav_open_memory_write() to open the stream from a file or
// from a block of memory respectively.
//
// This is different from drwav_init_write() in that it will allocate the drwav object for you via
// DRWAV_MALLOC() before initializing it.
//
...
//
// This holds the internal FILE object until drwav_uninit() is called. Keep this in mind if you're caching
// drwav objects because the operating system may restrict the number of file handles an application can have
...
//
// This holds the internal FILE object until drwav_close() is called. Keep this in mind if you're caching
// drwav objects because the operating system may restrict the number of file handles an application can have
// open at any given time.
drwav *drwav_open_file(const char *filename);
drwav *
drwav_open_file_ex(const char *filename, drwav_chunk_proc onChunk, void *pChunkUserData, drwav_uint32 flags);
...
static const drwav_uint8 drwavGUID_W64_JUNK[16] = {0x6A,
                                                   0x75,
                                                   0x6E,
                                                   0x6B,
                                                   0xF3,
                                                   ...
                                                   0x8E,
                                                   0xDB,
                                                   0x8A}; // 6B6E756A-ACF3-11D3-8CD1-00C04F8EDB8A
...
static const drwav_uint8 drwavGUID_W64_SMPL[16] = {0x73,
                                                   0x6D,
                                                   0x70,
                                                   0x6C,
                                                   0xF3,
...
drwav_bool32 drwav_init_write__internal(drwav *pWav,
                                        ...
                                        drwav_seek_proc onSeek,
                                        void *pUserData);
drwav *drwav_open_write__internal(const drwav_data_format *pFormat,
                                  drwav_uint64 totalSampleCount,
                                  drwav_bool32 isSequential,
                                  drwav_write_proc onWrite,
                                  drwav_seek_proc onSeek,
...
drwav_bool32 drwav_init_file_write__internal(drwav *pWav,
                                             const char *filename,
                                             const drwav_data_format *pFormat,
                                             drwav_uint64 totalSampleCount,
                                             drwav_bool32 isSequential) {
...
drwav *drwav_open_file(const char *filename) { return drwav_open_file_ex(filename, NULL, NULL, 0); }
...
drwav_open_file_ex(const char *filename, drwav_chunk_proc onChunk, void *pChunkUserData, drwav_uint32 flags) {
    ...
    drwav *pWav = drwav_open_ex(drwav__on_read_stdio,
                                drwav__on_seek_stdio,
                                onChunk,
                                (void *)pFile,
                                pChunkUserData,
...
drwav *drwav_open_file_write__internal(const char *filename,
                                       const drwav_data_format *pFormat,
                                       drwav_uint64 totalSampleCount,
                                       drwav_bool32 isSequential) {
    FILE *pFile = drwav_fopen(filename, "wb");
    ...
    drwav *pWav = drwav_open_write__internal(pFormat,
                                             totalSampleCount,
                                             isSequential,
                                             drwav__on_write_stdio,
                                             drwav__on_seek_stdio,
...
drwav_bool32 drwav_init_memory_write__internal(drwav *pWav,
                                               void **ppData,
                                               size_t *pDataSize,
                                               const drwav_data_format *pFormat,
                                               drwav_uint64 totalSampleCount,
    ...
    *pDataSize = 0;
    ...
    if (!drwav_init_write__internal(pWav,
                                    pFormat,
                                    totalSampleCount,
                                    isSequential,
...
drwav *drwav_open_memory_ex(const void *data,
    ...
    drwav *pWav = drwav_open_ex(drwav__on_read_memory,
                                drwav__on_seek_memory,
                                onChunk,
                                (void *)&memoryStream,
                                pChunkUserData,
...
drwav *drwav_open_memory_write__internal(void **ppData,
                                         size_t *pDataSize,
                                         const drwav_data_format *pFormat,
                                         drwav_uint64 totalSampleCount,
                                         drwav_bool32 isSequential) {
...
drwav_bool32 drwav__on_seek(drwav_seek_proc onSeek,
                            void *pUserData,
                            int offset,
...
static drwav_uint32 drwav_get_bytes_per_sample(drwav *pWav) {
    // The number of bytes per sample is based on the bits per sample or the block align. We prioritize
    // floor(bitsPerSample/8), but if this is zero or the bits per sample is not a multiple of 8 we need to
...
static drwav_uint32 drwav_get_bytes_per_pcm_frame(drwav *pWav) {
    // The number of bytes per frame is based on the bits per sample or the block align. We prioritize
    // floor(bitsPerSample*channels/8), but if this is zero or the bits per frame is not a multiple of 8 we
    // need to fall back to the block align.
...
drwav_bool32 drwav_init_ex(drwav *pWav,
    ...
    // The first 4 bytes can be used to identify the container. For RIFF files it will start with "RIFF" and
    // for w64 it will start with "riff".
    ...
    if (pWav->container == drwav_container_riff) {
        ...
        if (drwav__on_read(onRead, pReadSeekUserData, chunkSizeBytes, sizeof(chunkSizeBytes), &cursor) !=
        ...
        if (drwav__on_read(onRead, pReadSeekUserData, chunkSize, sizeof(chunkSize), &cursor) !=
            sizeof(chunkSize)) {
            return DRWAV_FALSE;
        }
    ...
    //
    // In order to correctly report each chunk back to the client we will need to keep looping until the end
    // of the file.
    drwav_bool32 foundDataChunk = DRWAV_FALSE;
    drwav_uint64 dataChunkSize = 0;

    // The next chunk we care about is the "data" chunk. This is not necessarily the next chunk so we'll need
    // to loop.
    drwav_uint64 chunkSize = 0;
    for (;;) {
        drwav_chunk_header header;
        drwav_result result =
            drwav__read_chunk_header(onRead, pReadSeekUserData, pWav->container, &cursor, &header);
        ...
        if (!sequential && onChunk != NULL) {
            drwav_uint64 callbackBytesRead =
                onChunk(pChunkUserData, onRead, onSeek, pReadSeekUserData, &header);

            // dr_wav may need to read the contents of the chunk, so we now need to seek back to the position
            // before we called the callback.
        ...
        // If at this point we have found the data chunk and we're running in sequential mode, we need to
        // break out of this loop. The reason for this is that we would otherwise require a backwards seek
        // which sequential mode forbids.
        ...
        if (pWav->container == drwav_container_riff) {
            if (drwav__fourcc_equal(header.id.fourcc, "fact")) {
                ...
                // The sample count in the "fact" chunk is either unreliable, or I'm not understanding it
                // properly. For now I am only enabling this for Microsoft ADPCM formats.
        ...
        if (pWav->container == drwav_container_riff) {
            if (drwav__fourcc_equal(header.id.fourcc, "smpl")) {
                unsigned char
                    smplHeaderData[36]; // 36 = size of the smpl header section, not including the loop data.
                if (chunkSize >= sizeof(smplHeaderData)) {
                    drwav_uint64 bytesJustRead = drwav__on_read(onRead,
                                                                pReadSeekUserData,
                                                                smplHeaderData,
                    ...
                    if (bytesJustRead == sizeof(smplHeaderData)) {
                        ...
                        for (drwav_uint32 iLoop = 0;
                             iLoop < pWav->smpl.numSampleLoops && iLoop < drwav_countof(pWav->smpl.loops);
                             ++iLoop) {
                            unsigned char smplLoopData[24]; // 24 = size of a loop section in the smpl chunk.
                            bytesJustRead = drwav__on_read(onRead,
                                                           pReadSeekUserData,
                                                           smplLoopData,
                                                           sizeof(smplLoopData),
                                                           &cursor);
            ...
            if (drwav__guid_equal(header.id.guid, drwavGUID_W64_SMPL)) {
                // This path will be hit when a W64 WAV file contains a smpl chunk. I don't have a sample file
                // to test this path, so a contribution is welcome to add support for this.
            }
        }
    ...
    // We may have moved passed the data chunk. If so we need to move back. If running in sequential mode we
    // can assume we are already sitting on the data chunk.
    if (!sequential) {
    ...
    // installer on the libsndfile website), it appears the total sample count libsndfile uses for MS-ADPCM is
    // incorrect. It would seem they are computing the total sample count from the number of blocks, however
    // this results in the inclusion of extra silent samples at the end of the last block. The correct way to
    // know the total sample count is to inspect the "fact" chunk, which should always be present for
    // compressed formats, and should always include the sample count. This little block of code below is only
    // used to emulate the libsndfile logic so I can properly run my correctness tests against libsndfile, and
...
drwav_bool32 drwav_init_write__internal(drwav *pWav,
    ...
    // Not currently supporting compressed formats. Will need to add support for the "fact" chunk before we
    // enable this.
    ...
    // The initial values for the "RIFF" and "data" chunks depends on whether or not we are initializing in
    // sequential mode or not. In sequential mode we set this to its final values straight away since they can
    // be calculated from the total sample count. In non- sequential mode we initialize it all to zero and
    // fill it out in drwav_uninit() using a backwards seek.
    drwav_uint64 initialDataChunkSize = 0;
    if (isSequential) {
        initialDataChunkSize = (totalSampleCount * pWav->fmt.bitsPerSample) / 8;

        // The RIFF container has a limit on the number of samples. drwav is not allowing this. There's no
        // practical limits for Wave64 so for the sake of simplicity I'm not doing any validation for that.
...
void drwav_uninit(drwav *pWav) {
    ...
    if (pWav->onWrite != NULL) {
        ...
        // Chunk sizes. When using sequential mode, these will have been filled in at initialization time. We
        // only need to do this when using non-sequential mode.
...
#ifndef DR_WAV_NO_STDIO
    // If we opened the file with drwav_open_file() we will want to close the file handle. We can know whether
    // or not drwav_open_file() was used by looking at the onRead and onSeek callbacks.
...
drwav_bool32 drwav_seek_to_sample(drwav *pWav, drwav_uint64 sample) {
    ...
    // For compressed formats we just use a slow generic seek. If we are seeking forward we just seek forward.
    // If we are going backwards we need to seek back to the start.
    if (drwav__is_compressed_format_tag(pWav->translatedFormatTag)) {
        // TODO: This can be optimized.

        // If we're seeking forward it's simple - just keep reading samples until we hit the sample we're
        // requesting. If we're seeking backwards, we first need to seek back to the start and then just do
        // the same thing as a forward seek.
        ...
        if (sample > pWav->compressed.iCurrentSample) {
            ...
            while (offset > 0) {
                ...
                if (pWav->translatedFormatTag == DR_WAVE_FORMAT_ADPCM) {
                    ...
                    assert(DRWAV_FALSE); // If this assertion is triggered it means I've implemented a new
...
drwav_uint64 drwav_read_s16__msadpcm(drwav *pWav, drwav_uint64 samplesToRead, drwav_int16 *pBufferOut) {
    ...
    while (samplesToRead > 0 && pWav->compressed.iCurrentSample < pWav->totalSampleCount) {
        ...
        while (samplesToRead > 0 && pWav->msadpcm.cachedSampleCount > 0 &&
            ...
            pBufferOut[0] =
                (drwav_int16)pWav->msadpcm.cachedSamples[drwav_countof(pWav->msadpcm.cachedSamples) -
        ...
        // If there's nothing left in the cache, just go ahead and load more. If there's nothing left to load
        // in the current block we just continue to the next loop iteration which will trigger the loading of
        // a new block.
        if (pWav->msadpcm.cachedSampleCount == 0) {
            if (pWav->msadpcm.bytesRemainingInBlock == 0) {
                ...
                if (pWav->channels == 1) {
                    ...
                    newSample0 =
                        ((pWav->msadpcm.prevSamples[0][1] * coeff1Table[pWav->msadpcm.predictor[0]]) +
                         (pWav->msadpcm.prevSamples[0][0] * coeff2Table[pWav->msadpcm.predictor[0]])) >>
                        8;
                    ...
                    drwav_int32 newSample1;
                    newSample1 =
                        ((pWav->msadpcm.prevSamples[1][1] * coeff1Table[pWav->msadpcm.predictor[1]]) +
                         (pWav->msadpcm.prevSamples[1][0] * coeff2Table[pWav->msadpcm.predictor[1]])) >>
                        8;
    ...
    while (samplesToRead > 0 && pWav->compressed.iCurrentSample < pWav->totalSampleCount) {
        ...
        while (samplesToRead > 0 && pWav->ima.cachedSampleCount > 0 &&
            ...
            pBufferOut[0] =
                (drwav_int16)pWav->ima
                    .cachedSamples[drwav_countof(pWav->ima.cachedSamples) - pWav->ima.cachedSampleCount];
            pWav->ima.cachedSampleCount -= 1;
        ...
        if (pWav->ima.cachedSampleCount == 0) {
            if (pWav->ima.bytesRemainingInBlock == 0) {
                ...
                // From what I can tell with stereo streams, it looks like every 4 bytes (8 samples) is for
                // one channel. So it goes 4 bytes for the left channel, 4 bytes for the right channel.
...
static unsigned short g_drwavAlawTable[256] = {
    0xEA80, 0xEB80, 0xE880, 0xE980, 0xEE80, 0xEF80, 0xEC80, 0xED80, 0xE280, 0xE380, 0xE080, 0xE180, 0xE680,
    0xE780, 0xE480, 0xE580, 0xF540, 0xF5C0, 0xF440, 0xF4C0, 0xF740, 0xF7C0, 0xF640, 0xF6C0, 0xF140, 0xF1C0,
    0xF040, 0xF0C0, 0xF340, 0xF3C0, 0xF240, 0xF2C0, 0xAA00, 0xAE00, 0xA200, 0xA600, 0xBA00, 0xBE00, 0xB200,
    0xB600, 0x8A00, 0x8E00, 0x8200, 0x8600, 0x9A00, 0x9E00, 0x9200, 0x9600, 0xD500, 0xD700, 0xD100, 0xD300,
    0xDD00, 0xDF00, 0xD900, 0xDB00, 0xC500, 0xC700, 0xC100, 0xC300, 0xCD00, 0xCF00, 0xC900, 0xCB00, 0xFEA8,
    0xFEB8, 0xFE88, 0xFE98, 0xFEE8, 0xFEF8, 0xFEC8, 0xFED8, 0xFE28, 0xFE38, 0xFE08, 0xFE18, 0xFE68, 0xFE78,
    0xFE48, 0xFE58, 0xFFA8, 0xFFB8, 0xFF88, 0xFF98, 0xFFE8, 0xFFF8, 0xFFC8, 0xFFD8, 0xFF28, 0xFF38, 0xFF08,
    0xFF18, 0xFF68, 0xFF78, 0xFF48, 0xFF58, 0xFAA0, 0xFAE0, 0xFA20, 0xFA60, 0xFBA0, 0xFBE0, 0xFB20, 0xFB60,
    0xF8A0, 0xF8E0, 0xF820, 0xF860, 0xF9A0, 0xF9E0, 0xF920, 0xF960, 0xFD50, 0xFD70, 0xFD10, 0xFD30, 0xFDD0,
    0xFDF0, 0xFD90, 0xFDB0, 0xFC50, 0xFC70, 0xFC10, 0xFC30, 0xFCD0, 0xFCF0, 0xFC90, 0xFCB0, 0x1580, 0x1480,
    0x1780, 0x1680, 0x1180, 0x1080, 0x1380, 0x1280, 0x1D80, 0x1C80, 0x1F80, 0x1E80, 0x1980, 0x1880, 0x1B80,
    0x1A80, 0x0AC0, 0x0A40, 0x0BC0, 0x0B40, 0x08C0, 0x0840, 0x09C0, 0x0940, 0x0EC0, 0x0E40, 0x0FC0, 0x0F40,
    0x0CC0, 0x0C40, 0x0DC0, 0x0D40, 0x5600, 0x5200, 0x5E00, 0x5A00, 0x4600, 0x4200, 0x4E00, 0x4A00, 0x7600,
    0x7200, 0x7E00, 0x7A00, 0x6600, 0x6200, 0x6E00, 0x6A00, 0x2B00, 0x2900, 0x2F00, 0x2D00, 0x2300, 0x2100,
    0x2700, 0x2500, 0x3B00, 0x3900, 0x3F00, 0x3D00, 0x3300, 0x3100, 0x3700, 0x3500, 0x0158, 0x0148, 0x0178,
    0x0168, 0x0118, 0x0108, 0x0138, 0x0128, 0x01D8, 0x01C8, 0x01F8, 0x01E8, 0x0198, 0x0188, 0x01B8, 0x01A8,
    0x0058, 0x0048, 0x0078, 0x0068, 0x0018, 0x0008, 0x0038, 0x0028, 0x00D8, 0x00C8, 0x00F8, 0x00E8, 0x0098,
    0x0088, 0x00B8, 0x00A8, 0x0560, 0x0520, 0x05E0, 0x05A0, 0x0460, 0x0420, 0x04E0, 0x04A0, 0x0760, 0x0720,
    0x07E0, 0x07A0, 0x0660, 0x0620, 0x06E0, 0x06A0, 0x02B0, 0x0290, 0x02F0, 0x02D0, 0x0230, 0x0210, 0x0270,
    0x0250, 0x03B0, 0x0390, 0x03F0, 0x03D0, 0x0330, 0x0310, 0x0370, 0x0350};

static unsigned short g_drwavMulawTable[256] = {
    0x8284, 0x8684, 0x8A84, 0x8E84, 0x9284, 0x9684, 0x9A84, 0x9E84, 0xA284, 0xA684, 0xAA84, 0xAE84, 0xB284,
    0xB684, 0xBA84, 0xBE84, 0xC184, 0xC384, 0xC584, 0xC784, 0xC984, 0xCB84, 0xCD84, 0xCF84, 0xD184, 0xD384,
    0xD584, 0xD784, 0xD984, 0xDB84, 0xDD84, 0xDF84, 0xE104, 0xE204, 0xE304, 0xE404, 0xE504, 0xE604, 0xE704,
    0xE804, 0xE904, 0xEA04, 0xEB04, 0xEC04, 0xED04, 0xEE04, 0xEF04, 0xF004, 0xF0C4, 0xF144, 0xF1C4, 0xF244,
    0xF2C4, 0xF344, 0xF3C4, 0xF444, 0xF4C4, 0xF544, 0xF5C4, 0xF644, 0xF6C4, 0xF744, 0xF7C4, 0xF844, 0xF8A4,
    0xF8E4, 0xF924, 0xF964, 0xF9A4, 0xF9E4, 0xFA24, 0xFA64, 0xFAA4, 0xFAE4, 0xFB24, 0xFB64, 0xFBA4, 0xFBE4,
    0xFC24, 0xFC64, 0xFC94, 0xFCB4, 0xFCD4, 0xFCF4, 0xFD14, 0xFD34, 0xFD54, 0xFD74, 0xFD94, 0xFDB4, 0xFDD4,
    0xFDF4, 0xFE14, 0xFE34, 0xFE54, 0xFE74, 0xFE8C, 0xFE9C, 0xFEAC, 0xFEBC, 0xFECC, 0xFEDC, 0xFEEC, 0xFEFC,
    0xFF0C, 0xFF1C, 0xFF2C, 0xFF3C, 0xFF4C, 0xFF5C, 0xFF6C, 0xFF7C, 0xFF88, 0xFF90, 0xFF98, 0xFFA0, 0xFFA8,
    0xFFB0, 0xFFB8, 0xFFC0, 0xFFC8, 0xFFD0, 0xFFD8, 0xFFE0, 0xFFE8, 0xFFF0, 0xFFF8, 0x0000, 0x7D7C, 0x797C,
    0x757C, 0x717C, 0x6D7C, 0x697C, 0x657C, 0x617C, 0x5D7C, 0x597C, 0x557C, 0x517C, 0x4D7C, 0x497C, 0x457C,
    0x417C, 0x3E7C, 0x3C7C, 0x3A7C, 0x387C, 0x367C, 0x347C, 0x327C, 0x307C, 0x2E7C, 0x2C7C, 0x2A7C, 0x287C,
    0x267C, 0x247C, 0x227C, 0x207C, 0x1EFC, 0x1DFC, 0x1CFC, 0x1BFC, 0x1AFC, 0x19FC, 0x18FC, 0x17FC, 0x16FC,
    0x15FC, 0x14FC, 0x13FC, 0x12FC, 0x11FC, 0x10FC, 0x0FFC, 0x0F3C, 0x0EBC, 0x0E3C, 0x0DBC, 0x0D3C, 0x0CBC,
    0x0C3C, 0x0BBC, 0x0B3C, 0x0ABC, 0x0A3C, 0x09BC, 0x093C, 0x08BC, 0x083C, 0x07BC, 0x075C, 0x071C, 0x06DC,
    0x069C, 0x065C, 0x061C, 0x05DC, 0x059C, 0x055C, 0x051C, 0x04DC, 0x049C, 0x045C, 0x041C, 0x03DC, 0x039C,
    0x036C, 0x034C, 0x032C, 0x030C, 0x02EC, 0x02CC, 0x02AC, 0x028C, 0x026C, 0x024C, 0x022C, 0x020C, 0x01EC,
    0x01CC, 0x01AC, 0x018C, 0x0174, 0x0164, 0x0154, 0x0144, 0x0134, 0x0124, 0x0114, 0x0104, 0x00F4, 0x00E4,
    0x00D4, 0x00C4, 0x00B4, 0x00A4, 0x0094, 0x0084, 0x0078, 0x0070, 0x0068, 0x0060, 0x0058, 0x0050, 0x0048,
...
static void drwav__pcm_to_s16(drwav_int16 *pOut,
...
static void drwav__ieee_to_s16(drwav_int16 *pOut,
                               const unsigned char *pIn,
                               size_t totalSampleCount,
                               unsigned int bytesPerSample) {
    if (bytesPerSample == 4) {
        ...
        // Only supporting 32- and 64-bit float. Output silence in all other cases. Contributions welcome for
        // 16-bit float.
        drwav_zero_memory(pOut, totalSampleCount * sizeof(*pOut));
        return;
    }
...
drwav_uint64 drwav_read_s16__pcm(drwav *pWav, drwav_uint64 samplesToRead, drwav_int16 *pBufferOut) {
    ...
    while (samplesToRead > 0) {
        drwav_uint64 samplesRead =
            drwav_read(pWav, drwav_min(samplesToRead, sizeof(sampleData) / bytesPerSample), sampleData);
        if (samplesRead == 0) {
            break;
        }
...
void drwav_s24_to_s16(drwav_int16 *pOut, const drwav_uint8 *pIn, size_t sampleCount) {
    int r;
    for (size_t i = 0; i < sampleCount; ++i) {
        int x = ((int)(((unsigned int)(((const unsigned char *)pIn)[i * 3 + 0]) << 8) |
                       ((unsigned int)(((const unsigned char *)pIn)[i * 3 + 1]) << 16) |
                       ((unsigned int)(((const unsigned char *)pIn)[i * 3 + 2])) << 24)) >>
                8;
        r = x >> 8;
        pOut[i] = (short)r;
    }
...
static void
drwav__pcm_to_f32(float *pOut, const unsigned char *pIn, size_t sampleCount, unsigned int bytesPerSample) {
    // Special case for 8-bit sample data because it's treated as unsigned.
    if (bytesPerSample == 1) {
...
static void
drwav__ieee_to_f32(float *pOut, const unsigned char *pIn, size_t sampleCount, unsigned int bytesPerSample) {
    if (bytesPerSample == 4) {
        for (unsigned int i = 0; i < sampleCount; ++i) {
...
drwav_uint64 drwav_read_f32__msadpcm(drwav *pWav, drwav_uint64 samplesToRead, float *pBufferOut) {
    // We're just going to borrow the implementation from the drwav_read_s16() since ADPCM is a little bit
    // more complicated than other formats and I don't want to duplicate that code.
    drwav_uint64 totalSamplesRead = 0;
...
drwav_uint64 drwav_read_f32__ima(drwav *pWav, drwav_uint64 samplesToRead, float *pBufferOut) {
    // We're just going to borrow the implementation from the drwav_read_s16() since IMA-ADPCM is a little bit
    // more complicated than other formats and I don't want to duplicate that code.
    drwav_uint64 totalSamplesRead = 0;
    drwav_int16 samples16[2048];
    while (samplesToRead > 0) {
...
#ifdef DR_WAV_LIBSNDFILE_COMPAT
    // It appears libsndfile uses slightly different logic for the u8 -> f32 conversion to dr_wav, which in my
    // opinion is incorrect. It appears libsndfile performs the conversion something like "f32 = (u8 / 256) *
    // 2 - 1", however I think it should be "f32 = (u8 / 255) * 2 - 1" (note the divisor of 256 vs 255). I use
    // libsndfile as a benchmark for testing, so I'm therefore leaving this block here just for my automated
...
static void drwav__pcm_to_s32(drwav_int32 *pOut,
                              const unsigned char *pIn,
                              size_t totalSampleCount,
                              unsigned int bytesPerSample) {
    // Special case for 8-bit sample data because it's treated as unsigned.
...
static void drwav__ieee_to_s32(drwav_int32 *pOut,
                               const unsigned char *pIn,
                               size_t totalSampleCount,
                               unsigned int bytesPerSample) {
    if (bytesPerSample == 4) {
...
drwav_int16 *drwav__read_and_close_s16(drwav *pWav,
                                       unsigned int *channels,
                                       unsigned int *sampleRate,
                                       drwav_uint64 *totalSampleCount) {
    drwav_assert(pWav != NULL);
    ...
    drwav_int16 *pSampleData =
        (drwav_int16 *)DRWAV_MALLOC((size_t)sampleDataSize); // <-- Safe cast due to the check above.
    if (pSampleData == NULL) {
        drwav_uninit(pWav);
...
float *drwav__read_and_close_f32(drwav *pWav,
                                 unsigned int *channels,
                                 unsigned int *sampleRate,
                                 drwav_uint64 *totalSampleCount) {
    drwav_assert(pWav != NULL);
    ...
    float *pSampleData =
        (float *)DRWAV_MALLOC((size_t)sampleDataSize); // <-- Safe cast due to the check above.
    if (pSampleData == NULL) {
        drwav_uninit(pWav);
        return NULL; // Failed to allocate memory.
    }
...
drwav_int32 *drwav__read_and_close_s32(drwav *pWav,
                                       unsigned int *channels,
                                       unsigned int *sampleRate,
                                       drwav_uint64 *totalSampleCount) {
    drwav_assert(pWav != NULL);
    ...
    drwav_int32 *pSampleData =
        (drwav_int32 *)DRWAV_MALLOC((size_t)sampleDataSize); // <-- Safe cast due to the check above.
    if (pSampleData == NULL) {
        drwav_uninit(pWav);
        return NULL; // Failed to allocate memory.
    }
...
drwav_int16 *drwav_open_and_read_pcm_frames_s16(drwav_read_proc onRead,
    ...
    drwav_uint64 totalSampleCount;
    drwav_int16 *result =
        drwav_open_and_read_s16(onRead, onSeek, pUserData, &channels, &sampleRate, &totalSampleCount);
    if (result == NULL) {
        return NULL;
    }
...
float *drwav_open_and_read_pcm_frames_f32(drwav_read_proc onRead,
    ...
    drwav_uint64 totalSampleCount;
    float *result =
        drwav_open_and_read_f32(onRead, onSeek, pUserData, &channels, &sampleRate, &totalSampleCount);
    if (result == NULL) {
        return NULL;
    }
...
drwav_int32 *drwav_open_and_read_pcm_frames_s32(drwav_read_proc onRead,
    ...
    drwav_uint64 totalSampleCount;
    drwav_int32 *result =
        drwav_open_and_read_s32(onRead, onSeek, pUserData, &channels, &sampleRate, &totalSampleCount);
    if (result == NULL) {
        return NULL;
    }
...
#ifndef DR_WAV_NO_STDIO
drwav_int16 *drwav_open_file_and_read_s16(const char *filename,
                                          unsigned int *channels,
                                          unsigned int *sampleRate,
                                          drwav_uint64 *totalSampleCount) {
    if (sampleRate) *sampleRate = 0;
...
drwav_int16 *drwav_open_memory_and_read_pcm_frames_s16(const void *data,
    ...
    drwav_uint64 totalSampleCount;
    drwav_int16 *result =
        drwav_open_memory_and_read_s16(data, dataSize, &channels, &sampleRate, &totalSampleCount);
    if (result == NULL) {
        return NULL;
    }
...
drwav_int32 *drwav_open_memory_and_read_pcm_frames_s32(const void *data,
    ...
    drwav_uint64 totalSampleCount;
    drwav_int32 *result =
        drwav_open_memory_and_read_s32(data, dataSize, &channels, &sampleRate, &totalSampleCount);
    if (result == NULL) {
        return NULL;
    }
...
void drwav_free(void *pDataReturnedByOpenAndRead) { DRWAV_FREE(pDataReturnedByOpenAndRead); }
...
//
// v0.9.0 - 2018-12-16
//   - API CHANGE: Add new reading APIs for reading by PCM frames instead of samples. Old APIs have been
//   deprecated and
...
//
// v0.1a - 2016-05-07
//   - Fixed a bug in drwav_open_file() where the file handle would not be closed if the loader failed to
//   initialize.
//
// v0.1 - 2016-05-04
