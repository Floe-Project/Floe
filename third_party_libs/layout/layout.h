...
LAY_EXPORT void lay_get_margins_ltrb(lay_context *ctx,
                                     lay_id item,
                                     lay_scalar *l,
                                     lay_scalar *t,
                                     lay_scalar *r,
...
LAY_EXPORT void
lay_set_margins_ltrb(lay_context *ctx, lay_id item, lay_scalar l, lay_scalar t, lay_scalar r, lay_scalar b);
...
LAY_STATIC_INLINE void lay_get_rect_xywh(const lay_context *ctx,
                                         lay_id id,
                                         lay_scalar *x,
                                         lay_scalar *y,
                                         lay_scalar *width,
...
void lay_run_context(lay_context *ctx) {
    ...
    if (ctx->count > 0) lay_run_item(ctx, 0);
}
...
static LAY_FORCE_INLINE void
lay_append_by_ptr(lay_item_t *LAY_RESTRICT pearlier, lay_id later, lay_item_t *LAY_RESTRICT plater) {
...
void lay_set_margins_ltrb(lay_context *ctx,
                          lay_id item,
                          lay_scalar l,
                          lay_scalar t,
                          lay_scalar r,
...
void lay_get_margins_ltrb(lay_context *ctx,
                          lay_id item,
                          lay_scalar *l,
                          lay_scalar *t,
                          lay_scalar *r,
...
static LAY_FORCE_INLINE void lay_arrange_stacked(lay_context *ctx, lay_id item, int dim, bool wrap) {
    ...
    while (start_child != LAY_INVALID_ID) {
        ...
        [[maybe_unused]] uint32_t squeezed_count = 0; // count of squeezable elements
        uint32_t total = 0;
    ...
    while (child != LAY_INVALID_ID) {
        ...
        switch (b_flags & LAY_HFILL) {
            case LAY_HCENTER:
                ...
                break;
            case LAY_RIGHT: child_rect[dim] += space - child_rect[2 + dim] - child_margins[wdim]; break;
            case LAY_HFILL:
...
static LAY_FORCE_INLINE void lay_arrange_overlay_squeezed_range(lay_context *ctx,
                                                                int dim,
                                                                lay_id start_item,
                                                                lay_id end_item,
                                                                lay_scalar offset,
...
static LAY_FORCE_INLINE lay_scalar lay_arrange_wrapped_overlay_squeezed(lay_context *ctx,
                                                                        lay_id item,
                                                                        int dim) {
    const int wdim = dim + 2;
    lay_item_t *pitem = lay_get_item(ctx, item);
    ...
    switch (flags & LAY_ITEM_BOX_MODEL_MASK) {
        ...
        case LAY_ROW:
            if ((flags & 1) == (uint32_t)dim) {
                ...
                const lay_vec4 rect = ctx->rects[item];
                lay_arrange_overlay_squeezed_range(ctx,
                                                   dim,
                                                   pitem->first_child,
                                                   LAY_INVALID_ID,
                                                   rect[dim],
