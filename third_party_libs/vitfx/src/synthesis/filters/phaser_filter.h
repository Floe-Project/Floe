  ...
  private:
    ...
    void process(const poly_float *audio_in, int num_samples) {
        ...
        for (int i = 0; i < num_samples; ++i) {
            ...
            tick<saturateResonance, saturateInput>(audio_in[i],
                                                   coefficient,
                                                   current_resonance,
                                                   current_drive,
                                                   current_peak1,
    ...
    force_inline void tick(poly_float audio_in,
                           ...
                           poly_float peak3,
                           poly_float peak5) {
        poly_float filter_state_lows =
            remove_lows_stage_.tickBasic(allpass_output_, utils::min(coefficient * kClearRatio, 0.9f));
        poly_float filter_state_highs =
            remove_highs_stage_.tickBasic(filter_state_lows, coefficient * (1.0f / kClearRatio));
