...
class SallenKeyFilter : public Processor, public SynthFilter {
  public:
    ...
    void process12(const poly_float *audio_in,
                   int num_samples,
    ...
    void process24(const poly_float *audio_in,
                   int num_samples,
                   poly_float current_resonance,
                   poly_float current_drive,
                   poly_float current_post_multiply,
    ...
    void processDual(const poly_float *audio_in,
                     int num_samples,
                     poly_float current_resonance,
                     poly_float current_drive,
                     poly_float current_post_multiply,
    ...
    force_inline void tick(poly_float audio_in,
                           ...
                           poly_float stage1_feedback_mult,
                           poly_float drive,
                           poly_float normalizer);

    force_inline void tick24(poly_float audio_in,
                             poly_float coefficient,
                             poly_float resonance,
                             poly_float stage1_feedback_mult,
                             poly_float drive,
    ...
    poly_float getLowAmount24(int style) {
        if (style == kDualNotchBand) return high_pass_amount_;
        return low_pass_amount_;
    }
    poly_float getHighAmount24(int style) {
        if (style == kDualNotchBand) return low_pass_amount_;
