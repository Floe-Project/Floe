...
Output *SynthModule::createBaseModControl(std::string name,
                                          bool audio_rate,
                                          bool smooth_value,
...
Output *SynthModule::createMonoModControl(std::string name,
                                          bool audio_rate,
                                          bool smooth_value,
                                          Output *internal_modulation) {
    ...
    if (audio_rate) return control_rate_total;
...
Output *SynthModule::createPolyModControl(std::string name,
                                          bool audio_rate,
                                          bool smooth_value,
                                          Output *internal_modulation,
                                          Input *reset) {
    ...
    if (audio_rate) {
        ...
        if (reset) poly_total->useInput(reset, ModulationSum::kReset);
    } else
    ...
    if (details.value_scale == ValueDetails::kQuadratic) {
        ...
        Processor *exponential =
            new cr::ExponentialScale(details.min, details.max, 2.0f, details.post_offset);
        exponential->plug(control_rate_total);
...
Output *SynthModule::createTempoSyncSwitch(std::string name,
                                           Processor *frequency,
...
Output *SynthModule::getModulationSource(std::string name) {
    if (data_->mod_sources.count(name)) return data_->mod_sources[name];
...
const StatusOutput *SynthModule::getStatusOutput(std::string name) const {
    if (data_->status_outputs.count(name)) return data_->status_outputs.at(name).get();
...
Processor *SynthModule::getModulationDestination(std::string name, bool poly) {
    ...
    if (poly && poly_destination) return poly_destination;

    return getMonoModulationDestination(name);
}

Processor *SynthModule::getMonoModulationDestination(std::string name) {
    if (data_->mono_mod_destinations.count(name)) return data_->mono_mod_destinations[name];
...
Processor *SynthModule::getPolyModulationDestination(std::string name) {
    if (data_->poly_mod_destinations.count(name)) return data_->poly_mod_destinations[name];
...
ValueSwitch *SynthModule::getModulationSwitch(std::string name, bool poly) {
    if (poly) return getPolyModulationSwitch(name);
    return getMonoModulationSwitch(name);
}

ValueSwitch *SynthModule::getMonoModulationSwitch(std::string name) {
    if (data_->mono_modulation_switches.count(name)) return data_->mono_modulation_switches[name];

    for (SynthModule *sub_module : data_->sub_modules) {
        ValueSwitch *value_switch = sub_module->getMonoModulationSwitch(name);
        if (value_switch) return value_switch;
    }
...
ValueSwitch *SynthModule::getPolyModulationSwitch(std::string name) {
    if (data_->poly_modulation_switches.count(name)) return data_->poly_modulation_switches[name];
...
void SynthModule::addMonoProcessor(Processor *processor, bool own) {
    ...
    if (own) data_->owned_mono_processors.push_back(processor);


  getMonoRouter()->addIdleProcessor(processor);  / namespace vital
