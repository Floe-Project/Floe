...
class RandomGenerator {
  public:
    ...
    force_inline mono_float next() { return distribution_(engine_); }
    ...
    force_inline poly_float polyNext(poly_mask mask) {
        ...
        for (int i = 0; i < poly_float::kSize; ++i)
            if (mask[i]) result.set(i, next());
        return result;
    }

    force_inline void seed(int new_seed) { engine_.seed(new_seed); }
...
force_inline mono_float min(mono_float one, mono_float two) { return fmin(one, two); }

force_inline mono_float max(mono_float one, mono_float two) { return fmax(one, two); }
...
template <class T>
constexpr force_inline T pass(T input) {
    return input;
}

force_inline int imax(int one, int two) { return (one > two) ? one : two; }

force_inline int imin(int one, int two) { return (one > two) ? two : one; }

force_inline double interpolate(double from, double to, double t) { return t * (to - from) + from; }

force_inline mono_float interpolate(mono_float from, mono_float to, mono_float t) {
    return from + t * (to - from);
}

force_inline mono_float mod(double value, double *divisor) { return modf(value, divisor); }

force_inline mono_float mod(float value, float *divisor) { return modff(value, divisor); }
...
force_inline bool closeToZero(mono_float value) { return value <= kEpsilon && value >= -kEpsilon; }
...
force_inline mono_float centsToRatio(mono_float cents) { return powf(2.0f, cents / kCentsPerOctave); }

force_inline mono_float noteOffsetToRatio(mono_float cents) { return powf(2.0f, cents / kNotesPerOctave); }
...
force_inline bool isSilent(const mono_float *buffer, int length) {
    for (int i = 0; i < length; ++i)
        if (!closeToZero(buffer[i])) return false;
    return true;
}
...
force_inline mono_float inversePowerScale(mono_float t) { return 2.0f * logf((-t + 1.0f) / t); }

force_inline mono_float inverseFltScale(mono_float t) { return (t - 1.0f) / t; }
