...
- Change performance controls icon from 'sliders' to 'gaugue'. No sliders were actually used on Performance Controls panel.
...
- Show waveform and filter graph value tooltips clear of the whole graph instead of over it
- Add a tooltip to the draggable waveform loop region showing its start, end and crossfade values
- Make the LFO waveform display draggable: drag left or right to change the Time, up or down to change the Amount
- Add a quick animation for the key-range popup below the bottom panel
